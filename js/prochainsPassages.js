...
function getListeArretsLigne(codeLigne){
	...
	//var searchUrl = (url.otp=='local'?url.hostnameLocal+url.portWSTest:url.hostnameData);
	//var searchUrl += url.otp2()+'/index/routes/'+codeLigne.replace('_',':')+'/stops';
	...
	$.ajax({
		...
		$( document ).trigger( 'evtArretsLigneCharges', [codeLigne, zones] );
	});
}
